  ...
  export default {
    ...
    methods: {
      ...
      rowClick(row, column, event, tar) {
        ...
        let arr3 = this.findAllOrgPerson(
          allData,
          arr2[arr2.length - 1].orgId,
          "orgId",
          []
        );
        let arr4 = this.findChildOrg(allData, arr2[0].orgId, "parentId", []);
        let arr5 = this.findAllOrgPerson(
          allData,
          arr4[arr4.length - 1].orgId,
          "parentId",
          []
        );
        let arr6 = this.findChildOrg(allData, arr4[0].orgId, "parentId", []);
        let arr7 = this.findAllOrgPerson(allData, arr6[0].orgId, "parentId", []);
        console.log([
          ...arr1,
          ...arr2,
          ...arr3,
        ...
        return;
        console.log([...arr1]);
        let treeOrgs = this.getTreeList(allData, row.orgId, [], "orgId");
        let orgSortList = this.flatten(treeOrgs);
        let newList = [];
        allData.forEach((i) => {
          orgSortList.forEach((o) => {
            if (i.orgId === o.orgId) {
              newList.push(i);
            }
          });
          if (i.orgId === row.orgId) {
        ...
        let $rows = $tbody
          .find(".el-table__row")
          .slice(index + len, index + newList.length);
