    ...
    <div>
      <el-form :model="form" label-width="100px" size="small" v-if="form">
        ...
        <el-form-item label="播放规则：" v-else>
          <el-select
            v-model="form.resources_content.auto_play"
            placeholder="请选择"
          >
            <el-option
          ...
          <div class="upload-wrapper">
            ...
            >
              <template v-if="form.resources_content.urls.length">
                <div
                  class="video-wrapper"
                  v-for="(item, i) in form.resources_content.urls"
                  :key="i"
                >
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if (this.dialogData.type == "add") {
        ...
        if (this.dialogData.segementType == "测评") {
          ...
          switch_type = JSON.parse(
            JSON.stringify(
              this.$parent.$parent.dialogData.param
                .textbook_segment_data_details[this.dialogData.index]
                .resources_content.switch_type
            )
          );
          question_ids = JSON.parse(
            JSON.stringify(
              this.$parent.$parent.dialogData.param
                .textbook_segment_data_details[this.dialogData.index]
                .resources_content.question_ids
            )
          );
          this.form.score_config_id = switch_type;
          this.form.resources_content.question_ids = question_ids;
        } else {
          this.form.resources_content.urls = JSON.parse(
            JSON.stringify(
              this.$parent.$parent.dialogData.param
                .textbook_segment_data_details[this.dialogData.index]
                .resources_content.urls
            )
          );
          this.form.resources_content.auto_play = JSON.parse(
            JSON.stringify(
              this.$parent.$parent.dialogData.param
                .textbook_segment_data_details[this.dialogData.index]
                .resources_content.auto_play
            )
          );
        }
    ...
    dialogSave() {
      ...
      result.id = this.$parent.$parent.dialogData.param.textbook_segment_data_details[
        this.dialogData.index
      ].id;
      //6月23日早上修改
      result.score_config_id = form.score_config_id;
      result.segment_template_id = this.$parent.$parent.dialogData.param.textbook_segment_data_details[
        this.dialogData.index
      ].segment_template_id;
      this.$parent.$parent.form.textbook_segment_data_details[
        this.dialogData.index
      ] = result;
