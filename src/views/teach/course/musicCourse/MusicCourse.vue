<template>
  <div class="course-management">
    <el-form inline size="small" class="section-search">
      ...
      <el-form-item>
        <el-button v-permission="'CourseCreate'" type="success" plain size="small" @click="addClass">新增音乐课程</el-button>
      </el-form-item>
    </el-form>
    ...
    <!--<ul>-->
    <!--  <li><router-link :to="{name:'Course', query: {name:'english'}}" >英语课程</router-link></li>-->
    <!--  <li><router-link :to="{name:'Course', query: {name:'music'}}">音乐课程</router-link></li>-->
    <!--  <li><router-link :to="{name:'Course', query: {name:'art'}}">美术课程</router-link></li>-->
    <!--</ul>-->
  ...
  export default {
    ...
    methods: {
      ...
      async init() {
        ...
        let data = await this.ApiTeach.getMusicCourseListApi(json);
        this.classList = data.items;
        this.page.total = data.total;
      },
      //删除课程
      delCourse(id) {
        this.ApiTeach.delMusicCourseApi(id).then((res) => {
          this.init();
        });
      },
      handleSearch() {
      ...
      handleSwitch(id, val) {
        ...
        this.$confirm(`确定 ${_targetText} 课程？`, "提示", {
          ...
          .then(() => {
            ...
            this.ApiTeach.postMusicCourseStatusApi(param)
              .then((res) => {
